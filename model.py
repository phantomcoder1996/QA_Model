...
class CharacterEmbedding(nn.Module):
    ...
    def forward(self,x):
        ...
        mask_sum = torch.where(mask_sum > 0,mask_sum.float(),(torch.ones(mask_sum.size(),device=mask_sum.device)*1e-7)).reshape(-1,1)
...
class AttentiveReader(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.passage_contextual_embedding = rnn_cell(
            args.embedding_dim  + args.char_embed_layer_embedding_dim,
            args.hidden_dim,
            bidirectional=args.bidirectional,
            batch_first=True,
        ...
        self.question_contextual_embedding = rnn_cell(
            args.embedding_dim  + args.char_embed_layer_embedding_dim,
            args.hidden_dim,
            bidirectional=args.bidirectional,
            batch_first=True,
        ...
        # Initialize Context2Query (2)
        self.aligned_att = AlignedAttention(args.hidden_dim*2)
    ...
    def forward(self, batch):
        ...
        # Encode the concatenated embeddings for both the question and the answer to get contextual embeddings
        contextual_passage_embeddings = self.sorted_rnn(passage_embeddings,passage_lengths,self.passage_contextual_embedding) #[batch_size, p_len, hidden_dim*2]
        contextual_question_embeddings = self.sorted_rnn(question_embeddings,question_lengths,self.question_contextual_embedding) #[batch_size, q_len, hidden_dim*2]


        # 2) Context2Query: Compute weighted sum of question embeddings for
        #        each passage word and concatenate with passage embeddings.
        aligned_scores = self.aligned_att(
            contextual_passage_embeddings, contextual_question_embeddings, ~question_mask
        )  # [batch_size, p_len, q_len]
        aligned_embeddings = aligned_scores.bmm(contextual_question_embeddings)  # [batch_size, p_len, args.hidden_dim *2]
        passage_embeddings = cuda(
            self.args,
            torch.cat((contextual_passage_embeddings, aligned_embeddings), 2), # input is [batch_size, p_len, 4*hidden_dim ]
        )  # [batch_size, p_len, p_dim + q_dim]
        ...
        # 5) Question Attentive Sum: Compute weighted sum of question hidden
        #        vectors.
        question_scores = self.question_att(contextual_question_embeddings, ~question_mask)
        question_vector = question_scores.unsqueeze(1).bmm(contextual_question_embeddings).squeeze(1)
        question_vector = self.dropout(question_vector)  # [batch_size, q_hid]
